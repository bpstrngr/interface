...
function restartBlink(cm) {
  ...
  if (cm.options.cursorBlinkRate > 0) display.blinker = setInterval(() => display.cursorDiv.style.visibility = (on = !on) ? "" : "hidden", cm.options.cursorBlinkRate); else if (cm.options.cursorBlinkRate < 0) display.cursorDiv.style.visibility = "hidden";
}
...
function leftButtonStartDrag(cm, event, pos, behavior) {
  ...
  let dragEnd = operation(cm, e => {
    ...
    if (!moved) {
      ...
      if (webkit || ie && ie_version == 9) setTimeout(() => {
